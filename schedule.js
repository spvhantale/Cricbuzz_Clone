...
window.addEventListener("load", async (event) => {
  const url =
    "https://unofficial-cricbuzz.p.rapidapi.com/matches/get-schedules";
...
let appendFunc = (data) => {
  ...
  for (let el of data) {
    ...
    for (let i = 0; i < el.matchScheduleMap.matchScheduleList.length; i++) {
      ...
      let TimeZone =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].venueInfo
          .timezone;

      let SeriesName = el.matchScheduleMap.matchScheduleList[i].seriesName;

      let Team1 =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].team1.teamName;

      let Team2 =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].team2.teamName;

      let MatchType =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].matchDesc;

      let GroundName =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].venueInfo.ground;

      let City =
        el.matchScheduleMap.matchScheduleList[i].matchInfo[0].venueInfo.city;
      ...
      if (
        el.matchScheduleMap.matchScheduleList.length > 1 &&
        i !== el.matchScheduleMap.matchScheduleList.length - 1
      ) {
        let Hr = c("HR");
...
g("domestic").addEventListener("click", async (event) => {
  ...
  const url =
    "https://unofficial-cricbuzz.p.rapidapi.com/matches/get-schedules?matchType=domestic";
...
g("t20").addEventListener("click", async (event) => {
  ...
  const url =
    "https://unofficial-cricbuzz.p.rapidapi.com/matches/get-schedules?matchType=league";
...
g("women").addEventListener("click", async (event) => {
  ...
  const url =
    "https://unofficial-cricbuzz.p.rapidapi.com/matches/get-schedules?matchType=women";
